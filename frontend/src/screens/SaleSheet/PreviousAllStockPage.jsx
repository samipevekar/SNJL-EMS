...
import {selectShops} from '../../redux/slice/shopSlice';

const PreviousAllStockPage = () => {
  ...
  // State for selection modals and form
  ...
  useEffect(() => {
    if (updateStatus === 'succeeded') {
      ...
      if (selectedShopId) {
        dispatch(fetchStockIncrementBrands(selectedShopId));
      }
      dispatch(resetUpdateStockStatus());
      Alert.alert('Success', 'Stock updated successfully');
    }
  }, [updateStatus, dispatch, selectedShopId]);

  // Get shop options based on user role
  const getShopOptions = () => {
    if (user?.role === 'manager' && user?.assigned_shops?.length > 0) {
      return shops.filter(shop => user.assigned_shops.includes(shop.shop_id));
    }
    return shops;
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.headerTxt}>Previous All Stocks</Text>
      </View>

      {/* Shop Selection and Fetch Button */}
      <View style={styles.shopSelectionContainer}>
        ...
        >
          <Text style={styles.fetchButtonText}>Fetch Stocks</Text>
        </TouchableOpacity>
      </View>
        ...
        ListEmptyComponent={
          <View style={styles.centerContainer}>
            <Text style={styles.emptyText}>
              {selectedShopId ? 'No stock records found' : 'Please select a shop and fetch stocks'}
            </Text>
          </View>
        }
      />
...
export default PreviousAllStockPage;